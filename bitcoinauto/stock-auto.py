...
myToken = "xoxb-6429405771830-6421713538023-bW3IBsICQO0xmFn0zCCWz9jr"

def dbgout(message):
    """인자로 받은 문자열을 파이썬 셸과 슬랙으로 동시에 출력한다."""
    print(datetime.now().strftime('[%m/%d %H:%M:%S]'), message)
    strbuf = datetime.now().strftime('[%m/%d %H:%M:%S] ') + message
    post_message(myToken,"#stock-auto", strbuf)
...
def get_target_price(code):
    ...
    try:
        ...
        target_price = today_open + (lastday_high - lastday_low) * 0.5      # 목표가
        return target_price
...
if __name__ == '__main__': 
    try:
        symbol_list = ['252670', '114800', '251340','233740']     #종목 리스트
        ...
        while True:
            ...
            t_start = t_now.replace(hour=9, minute=0, second=0, microsecond=0)     #자동매매 개시시간
            t_sell = t_now.replace(hour=15, minute=15, second=0, microsecond=0)     #자동매매 매도 종료시간
            t_exit = t_now.replace(hour=15, minute=20, second=0,microsecond=0)     #자동매매 종료시간
